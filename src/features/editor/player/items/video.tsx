...
/**
 * Video component that handles both remote URLs and local blob URLs
 * Uses HTML5 video for blob URLs since Remotion's OffthreadVideo doesn't support them
 */
export const Video = ({
  item,
  options
  ...
  const isBlobUrl = details.src?.startsWith("blob:");
      ...
      >
        ...
        >
          <div style={calculateMediaStyles(details, crop)}>
            {isBlobUrl ? (
              // Use Remotion's Video component for blob URLs (uses native HTML5 video)
              <RemotionVideo
              ...
              // Use OffthreadVideo for remote URLs (better performance)
              <OffthreadVideo
                startFrom={(item.trim?.from! / 1000) * fps}
                endAt={(item.trim?.to! / 1000) * fps || 1 / fps}
                playbackRate={playbackRate}
                ...
                volume={details.volume || 0 / 100}
                onError={(e) => {
                  console.warn("OffthreadVideo playback error:", e);
                }}
              />
            )}
          </div>
